...
def check_ext(a):
    if not a[-4 : len(a)] == ".csv":
        raise argparse.ArgumentTypeError("file extension must be *.csv")
    return a
...
def read_user_cli_args():  # this doesn't act correctly when no args are given
    ...
    parser.add_argument(
        ...
        nargs=5,
        type=check_ext,
        help='enter 4 ".csv" files: emp_job, emp_dr, emp_gas, emp_add, cust_job',
    )
